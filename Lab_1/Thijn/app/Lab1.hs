...
cubelst x = foldr (\x acc ->  acc + x^3) 0 [1..n] where n = abs x

cubelst' :: Integer -> Integer
cubelst' x = div (n ^ 2 + n) 2 ^ 2 where n = abs x
...
doubled xs =  map (\(i, x) -> if not (even i) then x * 2 else x) (reverseIndexed xs)
